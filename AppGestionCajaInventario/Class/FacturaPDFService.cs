...
namespace AppGestionCajaInventario.Class
{
    public class FacturaPdfService
    {
        ...
        public void GenerarFacturaPdf(DocumentoResponseDto documento, string rutaArchivo)
        {
        ...
        private void AgregarEncabezado(Document doc, DocumentoResponseDto documento)
        {
            ...
            header.AddCell(new PdfPCell(new Phrase($"Factura Nº: {documento.NumeroDocumento}", FontNormal)) { Border = iTextSharp.text.Rectangle.NO_BORDER });
        ...
        private void AgregarCliente(Document doc, DocumentoResponseDto documento)
        {
            doc.Add(new Paragraph($"Cliente: {documento.ClienteNombre}", FontNormal));
            doc.Add(new Paragraph("\n"));
        }
        ...
        private void AgregarTablaProductos(Document doc, List<DetalleDocumentoDto> detalles)
        {
            ...
            table.AddCell(new PdfPCell(new Phrase("ProductoID", FontNormal)) { BackgroundColor = BaseColor.LIGHT_GRAY });
            table.AddCell(new PdfPCell(new Phrase("Precio Unitario", FontNormal)) { BackgroundColor = BaseColor.LIGHT_GRAY });
